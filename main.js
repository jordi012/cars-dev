...
ipcMain.handle('export-to-excel', async (event, cars) => {
  try {
    const workbook = new ExcelJS.Workbook();
    const worksheet = workbook.addWorksheet('Cars');

    // Add headers
    worksheet.addRow(['Title', 'Price', 'URL']);

    // Add data
    cars.forEach(car => {
      worksheet.addRow([car.title, car.price, car.url]);
    });

    // Create downloads directory if it doesn't exist
    const downloadsPath = path.join(app.getPath('downloads'), 'car_list.xlsx');
